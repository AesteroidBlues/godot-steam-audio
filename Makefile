install-steam-audio:
	curl -s https://api.github.com/repos/ValveSoftware/steam-audio/releases/latest \
		...
		| cut -d : -f 2,3 | tr -d \" | xargs curl -L -o src/lib/steamaudio.zip
	unzip src/lib/steamaudio.zip -d src/lib/
	rm src/lib/steamaudio.zip
	cp -rf src/lib/steamaudio/lib/linux-x64/* project/addons/godot-steam-audio/bin/
	cp -rf src/lib/steamaudio/lib/windows-x64/* project/addons/godot-steam-audio/bin/
	cp -rf src/lib/steamaudio/lib/osx/* project/addons/godot-steam-audio/bin/
	cp -rf src/lib/steamaudio/lib/android-armv8/* project/addons/godot-steam-audio/bin/android/arm64
	cp -rf src/lib/steamaudio/lib/android-x64/* project/addons/godot-steam-audio/bin/android/x86_64
	cp -rf src/lib/steamaudio/lib/ios/* project/addons/godot-steam-audio/bin/ios/
